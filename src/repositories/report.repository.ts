...
export class ReportRepository {
  ...
  async reportAll(only_hotel: number = null, year?: number): Promise<any> {
    if (!year) {
      year = dayjs().year();
    }
    ...
    const totalHotels = only_hotel ? 1 : await this.hotelRepo.count();
    ...
    const totalAvailableRooms =
      roomStatusCounts.find((s) => s.status === RoomStatus.AVAILABLE)?.count ??
      0;
    const totalOccupiedRooms =
      roomStatusCounts.find((s) => s.status === RoomStatus.OCCUPIED)?.count ??
      0;
    const totalReservedRooms =
      roomStatusCounts.find((s) => s.status === RoomStatus.RESERVED)?.count ??
      0;

    // Booking counts
    const booking = await this.bookingRepo.count(
      only_hotel
        ? { where: { details: { room: { hotel: { id: only_hotel } } } } }
        : {},
    );
    const bookingDetail = await this.bookingDetailRepo.count(
    ...
    const monthlyQuery = this.bookingDetailRepo
      ...
      .addSelect('SUM(bd.brokerage_fees)', 'totalBrokerage')
      .addSelect('SUM(bd.total)', 'totalAll')
      .where('bd.checkin_date BETWEEN :start AND :end', {
        start: startOfYear,
        end: endOfYear,
      });
